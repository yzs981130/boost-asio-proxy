...
//	std::cout << "handle_resolve. Error: " << err.message() << "\n";
    if (!err) {
        ...
        fServer = endpoint_iterator->endpoint().address().to_string();
        //std::cout << fServer << std::endl;
...
std::string connection::dns_ip;
int32_t connection::dns_port;